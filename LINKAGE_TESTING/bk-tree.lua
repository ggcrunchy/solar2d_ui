...
-- ^^^ :/ Save #s1, #s2 in variables
-- TODO: can we early out if trivially > n? (need to read up here)
	local t = {}
	for i=1, #s1+1 do
		t[i] = {i-1} -- seed previous entry in each row
	end

	for i=1, #s2+1 do
		t[1][i] = i-1 -- seed last row for first time
	end

	for i=2, #s1+1 do
	
		for j=2, #s2+1 do
			local cost = (s1:sub(i-1,i-1) == s2:sub(j-1,j-1) and 0) or 1
			t[i][j] = min(
				t[i-1][j] + 1, -- last row
				t[i][j-1] + 1, -- previous entry
				t[i-1][j-1] + cost) -- previous entry in last row
		end
	end
-- ranges would make more sense, and much less offseting, with i-1 -> i and i -> i + 1, etc.
-- ^^ TODO: maintain this with one row, a couple "last entry" variables
-- maybe break out first row and then rest?
-- no need for custom min
	return t[#s1+1][#s2+1]
...
function bk_tree:new(root_word, dist_func)
	...
	local n_obj = {}
	if root_word then n_obj.root = { str = root_word, children = {} } end -- TODO: lazy children
...
function bk_tree:insert(word, node)
	...
	if not node then
		self.root = { str = word, children = {} } -- TODO: lazy children
		return true
	end	
	...
	local some_node = node.children[dist] -- TODO: lazy children

	if not some_node then
		node.children[dist] = { str = word, children = {} } -- TODO: ditto
		return true
	end	

	return self:insert(word, some_node) -- TODO: AuxInsert
...
function bk_tree:query(word, n, node, matches)
	...
	local dist = self.dist_func(word, node.str)
	if dist <= n then matches[#matches+1] = {str = node.str, distance = dist} end -- TODO: return as { str1, dist1, str2, dist2, ... }
	
	for k, child in pairs(node.children) do
		if child ~= nil then -- ????????
			if k >= dist-n and k <= dist+n then -- if n small, could just iterate these indices
				self:query(word, n, child, matches) -- TODO: AuxQuery
			end
		end
	end
...
function bk_tree:query_sorted(word, n)
	...
	table.sort(result, function(a,b) return a.distance < b.distance end) -- TODO: break out
